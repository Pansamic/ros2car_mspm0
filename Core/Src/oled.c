...
void OLED_W_RES(uint8_t BitValue)
{
    ...
    if(BitValue == 0){
        DL_GPIO_clearPins(GPIO_GRP_OLED_PORT, GPIO_GRP_OLED_PIN_RESET_PIN);
    } else {
        DL_GPIO_setPins(GPIO_GRP_OLED_PORT, GPIO_GRP_OLED_PIN_RESET_PIN);
    }
}
...
void OLED_W_DC(uint8_t BitValue)
{
    ...
    if(BitValue == 0){
        DL_GPIO_clearPins(GPIO_GRP_OLED_PORT, GPIO_GRP_OLED_PIN_DC_PIN);
    } else {
        DL_GPIO_setPins(GPIO_GRP_OLED_PORT, GPIO_GRP_OLED_PIN_DC_PIN);
    }
}
...
void OLED_W_CS(uint8_t BitValue)
{
    ...
    if(BitValue == 0){
        DL_SPI_setChipSelect(SPI_0_INST, DL_SPI_CHIP_SELECT_1);
    } else {
...
/**
  ...
  * 返 回 值：无
  */
static inline void OLED_SPI_SendByte(uint8_t Byte)
{
...
void OLED_Init(void)
{
    OLED_W_RES(0);				//拉低RES，复位OLED
    for(uint32_t i=0 ; i<0XFFF ; i++){} //延时一段时间
    OLED_W_RES(1);				//拉高RES，结束复位
...
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, char *format, ...)
{
    ...
    lwvsnprintf(String, sizeof(String), format, arg);			//使用vsprintf打印格式化字符串和参数列表到字符数组中
    va_end(arg);							//结束变量arg
